...
export class TransferDonationComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.userRole === 'MARKETING_KOMUNIKASI' || this.userRole === 'FRONT_OFFICE' || this.userRole === 'TIM_JEMPUT_DONASI') {
  ...
  onAddTransferDonation(addForm: NgForm) {
    ...
    this.transferDonationService.addTransferDonation(formData).pipe(
      ...
      () => {
        if (this.userRole === 'FRONT_OFFICE' || this.userRole === 'TIM_JEMPUT_DONASI' || this.userRole === 'MARKETING_KOMUNIKASI') {
          this.getTransferDonationsByUserId(this.userId);
        } else {
          this.getAllTransferDonations();
        }
  ...
  openViewImage(content: any, transferDonation: TransferDonation) {
    this.modal.open(content)
    this.transferDonation = transferDonation;
    this.url = transferDonation.receiptPhoto;
  ...
  onSearch(search: string) {
    ...
    this.transferDonations = this.originalTransferDonations.filter((transferDonation: TransferDonation) => {
      ...
      const amountMatch = transferDonation.amount.toLocaleString('id', { minimumFractionDigits: 0, maximumFractionDigits: 0 }).includes(searchValue);
      ...
      const otherInfoMatch = transferDonation.otherInfo?.toLowerCase().includes(searchValue);
      const donorMatch = transferDonation.donor.name.toLowerCase().includes(searchValue);
      return dateMatch || contractMatch || amountMatch || otherInfoMatch  || transfersAccountMatch || donorMatch;
    });
  }
